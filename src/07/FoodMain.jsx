...
export default function FoodMain() {
    ...
    const filteredData = foodFilterData.map((data, idx) => (<FoodCard infoData={data} key={idx} />));
    const handleAll = () => {        
    ...
    const showCategory = (item) => {        
        const selectedCat = FoodData.filter((data) => item === data["운영주체 분류"].replaceAll(" ", ""));
    ...
    return (
        <div className="w-full h-full mt-10 flex flex-col justify-start items-center p-5">
            <div className="w-full m-2 flex flex-row justify-between items-center gap-2">
                <TailButton bColor="green" caption="전체" onHandle={handleAll} />
                {categoryButtons}</div>
            <div className="w-full grid grid-cols-2 grid-rows-[200px] gap-2 ">
                {filteredData}
                {/* {
